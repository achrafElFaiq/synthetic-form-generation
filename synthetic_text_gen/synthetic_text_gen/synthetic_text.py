...
#Créé des images du textes et applique certaines déformations
...
class SyntheticText:
    ...
    def cleanFonts(self,outfile):
        ...
        for i in range(len(self.fonts)):
            self.fonts[i] = self.fonts[i].split('/')[-1]
        for i, filename in enumerate(self.fonts):
            try:
                ...
                print(e)
                hasChars, hasLower, hasNums,hasBrackets = (False, False, False,False)
            if hasChars:
